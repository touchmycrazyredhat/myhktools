...
function fnCheckUrl(url,oArgs)
{
	...
	EventEmitter.call(_t);
  	...
  	_t.setMaxListeners(_t.getMaxListeners() + 1 + n_maxLs);
	...
	fnSocket:function(h,p,szSend,fnCbk)
	{
		...
		try{
			if(h && !(s = rIp.exec(h)))
			{
				...
				{
					s = child_process.execSync("/sbin/ping -c 1 -t 1 " + h + " 2>&1");
					s = rIp.exec(s);
			...
			_t.tmClose(client,oTmp && oTmp.timeout||2000);
		}catch(e)
		{
			_t.g_szError += url + "\n";
			...
			if(a)
			{
				...
				for(var k in a)
				{
					...
					try{
						...
						{
							if(!global.g_noRepeat[url+o.tags])
							global.g_noRepeat[url+o.tags]=1,_t.info("start " + o.tags + ": " + url),o.doCheck(url,function(o9,_t1)
							{
		...
		if(s && url)
		{
			...
			if(a &&  0 < a.length &&  a[1])
			{
				...
				if(-1 < szUrl.indexOf(".do/logon.do"))
					console.log("注意，url存问题，请反馈开发者优化爬虫：" + szUrl)
			}
			...
			while(a = aHref.exec(s))
			{
				...
				{
					szUrl = szUrl.replace(/\/*[^\/]*$/gmi,'') + "/" + a[1];
					if(-1 < szUrl.indexOf(".do/logon.do"))
						console.log("注意1，url存问题，请反馈开发者优化爬虫：" + szUrl)
				}
				aTmp.push(szUrl);
