...
function fnCheckUrl(url,oArgs)
{
	...
	*/
	fnSocket:function(h,p,szSend,fnCbk)
	{
		...
		try{
			...
			const client = net.connect(_t.fnOptHeader({"port": p,"host":h}), () => 
			{
			  client.write(szSend);
			});
			client.on('data', (data) => 
			...
			if(a)
			{
				...
				for(var k in a)
				{
					...
					try{
						...
						{
							_t.log("start " + o.tags + ": " + url);
							o.doCheck(url,function(o9,_t1)
							{
								g_oForm[o9.url + _t1.tags] = 1;
								// 便于差分比较结果
								// g_oRstAll[o9.url] = o9;
								fnCbk(o9,_t1);
							},parms);
	...
	*/
	...
	fnDoForm:function(s,url)
	{
		...
		if(s && url)
		{
			...
			// console.log(oFFds);
			if(0 < n)_t.runChecks(szUrl,"parms",null,oFFds);
			...
			if(0 < aTmp.length)
			{
				...
				async.mapLimit(aTmp,_t.g_nThread,function(s,fnCbk1)
				{
					...
					_t.runChecks(s,"web,struts2",function(o,_t)
					{
						_t.fnReport(o,_t);
					});
					fnCbk1();
	...
	*/
	...
	_t.runChecks(url,"weblogic,struts2,web");
	// 加载所有的插件动态库
	// 各种插件库分开编写，便于维护
