...
function fnCheckUrl(url,oArgs)
{
	...
	_t.getTimeCur = getTimeCur;
  	...
  	function error(e,t)
	{
		if(program.verbose && e)_t.emit('error',e,t,_t);
	}
	// 日志
	function log(s)
	{
...
r.on('ready',function()
{
	...
	*/
	...
	fnGetRegMath:function(s,r,n,oM)
	{
		...
		while(a = r.exec(s))
		{
			if(o[a[n]] || -1 < a[n].indexOf("encodeURI") || -1 < a[n].indexOf("href") || 'url' == a[n])continue;
			o[a[n]] = 1;
			k.push(a[n]);
		}
		o = null;
	...
	fnDoForm:function(s,url,rep,t)
	{
		...
		if(s && url)
		{
			...
			// 有些输入对象只有id
			var re = /<input .*?name=['"]*([^\s'"]+)['"]*\s[^>]*>/gmi,oFFds = {},n = 0,
			    reFa = /action=['"]*([^\s"'><]+)['"]*/gmi,
			...
			// 连接的遍历、获取、探测
			var aTmp = [];
			aTmp.push1=function(s){if(s && 14 < s.length)this.push(s);};
			var oT1 = {};
			...
			aRst = aRst.concat(_t.fnGetRegMath(s,aHref,1,oT1));
