...
function fnCheckUrl(url,oArgs)
{
	...
	_t.getTimeCur = getTimeCur;
  	...
  	function error(e,t)
	{
		_t.emit('error',_t.getTimeCur().bgRed+e,t,_t);
	}
	// 日志
	function log(s)
	{
		if(program.verbose)
		_t.emit('log',_t.getTimeCur().bgBlue + s,_t);
	}
	// 日志
	function info(s)
	{
		if(program.verbose)
		_t.emit('info',_t.getTimeCur().bgGreen + s,_t);
	}
	...
	*/
	...
	tmClose:function(o,t)
	{
		setTimeout(function()
		{
			if(o)(o['end'] || o['destroy']|| o['close']||function(){})();
		},t);
	},
	g_szError:'',
	...
	*/
	fnSocket:function(h,p,szSend,fnCbk)
	{
		...
		try{
			...
			client.on('data', (data) => 
			{
				fnCbk(global.g_mysocket[sKey] = data);
				client.end();
				// _t.info("收到数据: " + h + ":" + p + "\n" + data.toString());
			});
			client.on('end', () =>
