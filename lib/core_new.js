...
{
	// 全局初始化的标志
	var _t = this, g_bInit = 0,g_szCmd = '',g_szCmdW = '';
	...
	// 初始化
  	function fnInit()
  	{
		...
		g_szCmd = _t.g_szCmd = program.cmd||"echo whoami:;whoami;echo pwd:;pwd;echo cmdend";
		g_szCmdW = _t.g_szCmdW = program.cmd||"echo whoami:&& whoami && echo pwd:&& echo %cd% && echo cmdend"; // && dir
		console.log(g_szCmd);
		_t.emit('init');
		if(!url)
		{
