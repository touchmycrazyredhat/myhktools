...
var Wire = function(_stream$Duplex) {
  ...
  Wire.prototype._requestPiece = function _requestPiece(piece) {
    var msg = Buffer.concat([new Buffer([BT_MSG_ID]), new Buffer([this._ut_metadata]), _bencode2.default.encode({
      msg_type: 0,
      piece: piece
    })]);
  ...
  Wire.prototype._sendExtHandshake = function _sendExtHandshake() {
    var msg = Buffer.concat([new Buffer([BT_MSG_ID]), new Buffer([EXT_HANDSHAKE_ID]), _bencode2.default.encode({
      m: {
        ut_metadata: 1
      }
    })]);
  ...
  Wire.prototype._onDone = function _onDone(metadata) {
    ...
    this.emit('metadata', {
      info: _bencode2.default.decode(metadata)
    }, this._infohash);
  };
